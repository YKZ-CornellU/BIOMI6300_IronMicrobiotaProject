...
```{r Metadata}
metadata_physeq <- read.delim(
  "/local/workdir/yz3272/BIOMI6300_IronMicrobiotaProject/metadata.csv",
  stringsAsFactors = FALSE,
  check.names = FALSE
)

head(metadata_physeq)
colnames(metadata_physeq)

rownames(metadata_physeq) <- metadata_physeq[[1]]
metadata_physeq <- metadata_physeq[, -1, drop = FALSE]

metadata_physeq <- metadata_physeq[colnames(final_ASV_table), , drop = FALSE]

all(rownames(metadata_physeq) == colnames(final_ASV_table))

raw_physeq <- phyloseq(
  otu_table(final_ASV_table, taxa_are_rows = TRUE),
  tax_table(as.matrix(ASV_tax_table)),
  sample_data(metadata_physeq)
)
